...
Conflict recovery actions: `Use remote version` (apply `syncConflict_.remote`, clear conflict, store its revision) and `Overwrite remote version` (confirm, then `PUT` with `If-Match: "<current remote revision>"` from the conflict record, or `If-None-Match: *` when the remote is a tombstone). `Retry sync` re-runs the pending write for that tag with its stored revision.
